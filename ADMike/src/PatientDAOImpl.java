	...
	public List<Patient> getSearchResult(final Search patientSearch) {
		...
		ParamMapper paramMapper = new ParamMapper() {
			...
			public void mapParam(PreparedStatement preparedStatement)
					throws SQLException {
				int i = 0;
				if (patientSearch.getSearchValue() != null) {
					...
					preparedStatement.setString(++i, patientSearch.getSearchValue().replaceAll("\\s+", " ").replaceAll("\\s", "%").trim().toUpperCase().concat("%"));
					String tempp = patientSearch.getSearchValue().replaceAll("\\s+", " ").replaceAll("\\s", "%").trim().toUpperCase().concat("%");
					System.out.println("@@ PatientDAOImpl: "+tempp);
				...
				if (patientSearch.getFacilityId() > 0) {
					preparedStatement.setLong(++i,
							...
							System.out.println("@@ PatientDAOImpl: "+patientSearch.getFacilityId());
				}
				// timc
		...
		if (null != patientSearch.getSearchValue() || !patientSearch.getSearchValue().equals(" ")) {
			whereClause.append("UPPER(DP.FULL_NAME) LIKE ?");
			System.out.println(":"+ patientSearch.getSearchValue());
		}
...
//	public Patient getPatientDetails(final long facilityId,	final long spectraMRN) {
	public Patient getPatientDetails(final long facilityId,	final long spectraMRN, final String patientDOB) {
		...
		try {
			System.out.println("GET_PATIENT_RESULT_DETAILS "+ query.toString());
	...
	public Patient getPatientReqInfo(final String facilityNum,
		...
		try {
			System.out.println("Patient: GET_PATIENT_REQ_DETAILS "+ GET_PATIENT_REQ_DETAILS);
